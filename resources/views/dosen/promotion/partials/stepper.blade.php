<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
    <div class="p-6 text-gray-900">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Riwayat Proses Pengajuan Anda</h3>
        <ol class="relative border-l border-gray-200">
            ...
            <li class="mb-10 ml-6">
                ...
                <time class="block mb-2 text-sm font-normal leading-none text-gray-400">
                    Diproses pada {{ $log->created_at->translatedFormat('l, d F Y \p\u\k\u\l H:i') }}
                </time>
                @if($log->catatan)
                <p class="mb-4 text-sm font-normal text-gray-500 p-3 bg-gray-50 rounded-md border">
                    <strong>Catatan dari Tim Pemroses:</strong> {{ $log->catatan }}
                </p>
                @endif
            </li>
            @empty
            <li class="ml-6">
                <p class="text-sm text-gray-500">Belum ada riwayat proses. Pengajuan Anda sedang disiapkan.</p>
            </li>
            @endforelse
        </ol>
    </div>
