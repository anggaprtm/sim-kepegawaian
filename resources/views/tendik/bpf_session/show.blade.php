<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Detail Sidang BPF: {{ $session->nama_sesi }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex space-x-2">
                <a href="{{ route('tendik.bpf_session.invitation', $session->id) }}" class="px-4 py-2 bg-blue-500 text-white rounded-md">Unduh Undangan</a>
                {{-- Tambah tombol presensi & notula --}}
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('tendik.bpf_session.process', $session->id) }}" method="POST">
                        @csrf
